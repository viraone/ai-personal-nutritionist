...
app.post("/api/kroger/order", async (_req, res) => {
  try {
    ...
    if (!token) return res.status(401).json({ error: "Not connected to Kroger yet.", needLogin: true });
    if (!history.length) return res.status(400).json({ error: "Talk to Maya first so she can suggest foods!" });
